...
class Evaluator(object):
	...
	def eval_board(self, player, game):
		...
		for unit in player.units:
			...
			if unit.has_component(Attack):
				self.soldiers.add(unit)
			if unit.has_component(Spawner):
				self.spawners.add(unit)
		...
		self.eval_units(player, influence_map, threat_map)
		self.eval_enemy_units(enemy)
		self.eval_soldiers(enemy, game)
		self.eval_spawner_pos(player, enemy, game)
		self.eval_workers(threat_map, game, ResourceCategory.MINERAL)
	...
	def eval_units(self, player, influence_map, threat_map):
		need_workers = self.need_build_workers(influence_map, threat_map)
		for unit in player.units:
	...
	def eval_soldiers(self, enemy, game):
		...
		for soldier in self.soldiers:
			if not soldier.get_component(Attack)._get_attacks(game):
